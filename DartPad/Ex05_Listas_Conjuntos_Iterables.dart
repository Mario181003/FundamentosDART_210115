void main() {
  // La declaración de una list extra limitada por corchetes (brackets) y cada elemento individual y puede repetirse. 
  ...
  print("El tamaña: ${numeros.length}");
  print("""
  Para acceder a los elementos debemos especificar la posicion entre []
  Para acceder a los elementos en la primera posicion es: ${numeros[0]}
  ...
  Es importante observar que la funcion reserved ha transfromado la LIST original y la 
  ...
  // Conjuto de datos (SET)
  // Un SET es una estructura de datos que utiliza DART, que simplifica los elementos eliminando los duplicados
  print("""
  Veamos que pasa con los Conjuntos (SET)
  Convertimos el ITERABLE en un SET usando .toSET: ${numerosAlreves.toSet()}
  Podemos visualizar en el resultado que el SET a eliminado los duplicados y ahora delimita datos
  usando {}""");
  
  // De igul manera odemos realizr operaciones del filtro de elementos usando .where()
  final numerosMayoresA5 = numeros.where((int num) {
    return num > 5;
  });
  
  print("""Hemos filtrado sobre la LIST solo los elementos mayores a 5:
  Resultados de Filtro: ${numerosMayoresA5}""");
